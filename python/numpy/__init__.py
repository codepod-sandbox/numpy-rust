...
def std(a, axis=None, dtype=None, out=None, ddof=0, keepdims=False, where=True, mean=None, correction=None):
    ...
    if isinstance(a, _MA):
        ...
        if isinstance(v, _MA):
            ...
            result = sqrt(v) if isinstance(v, ndarray) else _stdlib_math.sqrt(v)
        if out is not None and isinstance(out, _MA):
...
# Functions defined earlier in this file are referenced here by their
# original names; after this block, the module-level names become ufunc
# instances whose __call__ delegates to the saved reference.
